...
class TestNetworkAttributeFunctions(unittest.TestCase):
    ...
    def test_time_visibility_function(self):
        """
        Checks time visibility function (used to calculate how long each pair of satellites is visible to one another from current snapshot) - both basic and advanced (time extends beyond defined orbit and "wraps around")
        """
        # Create test data
    ...
    def test_cost_function(self):
        """
        Tests cost function returns correct results given a series of matrices representing the different network attributes taken into consideration.
        """
        ...
        self.assertTrue(np.allclose(satnet.cost_function(visibility, time_visibility, dist, sunlight, alpha=0.2, beta=0.3, gamma=0.5, total_satellites=3), np.asarray([[-1., 0.2, 0.32916667], [0.2, -1., 0.84], [0.32916667, 0.84, -1.]])))

# np.asarray([[0.0875, 0.2, 0.32916667], [0.2, 0.0875, 0.84], [0.32916667, 0.84, 0.0875]])
